...
int vetor_trocar(vetor *vec, int pos1, int pos2) {
	char* cache = vec->elementos[pos1].str;
	vec->elementos[pos1].str=vec->elementos[pos2].str;
	vec->elementos[pos2].str=cache;

	/*const char * elem1 = vetor_elemento(vec, pos1);

	const char * elem2 = vetor_elemento(vec, pos2);

	if(vetor_atribui(vec, pos1, elem2) == -1) {
		free(cache);
		cache = NULL;
		return -1;
	}

	if(vetor_atribui(vec, pos2, cache) == -1) {
		free(cache);
		cache = NULL;
		return -1;
	}
...
vetor * vetor_concatena(vetor *vec1, vetor *vec2) {
	int tamanho = vetor_tamanho(vec2);
	int i;

	for(i = 0; i < tamanho; i++) {
		vetor_insere(vec1, vetor_elemento(vec2, i), -1);
	}
...
int vetor_inverte(vetor *vec) {
	...
	for(i = 0; i < tamanho / 2; i++) {
		if(vetor_trocar(vec, i, tamanho - i - 1) == -1) {
			return -1;
		}
	}
