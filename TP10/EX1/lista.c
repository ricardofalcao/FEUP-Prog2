...
lista * lista_pesquisa_substring(lista *lst, char *substring) {
	lista *output = lista_nova();

	l_elemento *root = lista_inicio(lst);
	while(root != NULL) {
		if(strstr(root->str, substring) != NULL) {
			lista_insere(output, root->str, NULL);
		}
...
lista * lista_concatena(lista *lst1, lista *lst2) {
	...
	if(lst2) {
		l_elemento *fim = lista_fim(lst1);
		l_elemento *inicio = lista_inicio(lst2);

		fim->proximo = inicio;
		inicio->anterior = fim;

		lst1->tamanho += lista_tamanho(lst2);
		lst1->fim = lst2->fim; 
	}
...
int lista_insere_ordenada(lista *lst, char *inserir) {
	...
	while(root) {
		if(strcmp(inserir, root->str) < 0) {
			lista_insere(lst, inserir, root);
			return i;
		}
