...
int vetor_atribui(vetor* vec, int pos, const char* valor)
{
	...
	if(len_valor > strlen(vec->elementos[pos].titulo))
	{
		vec->elementos[pos].titulo = (char*)realloc(vec->elementos[pos].titulo, (len_valor+1)*sizeof(char));
		if(vec->elementos[pos].titulo == NULL)
		return -1;
	}

	/* copia valor */
	strcpy(vec->elementos[pos].titulo, valor);
...
const char* vetor_elemento(vetor* vec, int indice)
{
	...
	return vec->elementos[indice].titulo;
}
...
int vetor_ordena(vetor* vec)
{
	...
	{
		tmp = vec->elementos[i].titulo;
		for (j = i; j > 0 && strcmp(tmp, vec->elementos[j-1].titulo) < 0; j--)
		{
			vec->elementos[j] = vec->elementos[j-1];
		}
		vec->elementos[j].titulo = tmp;
	}
